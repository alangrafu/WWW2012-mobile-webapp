...
function getURL(url, word, attempts) {
  //var key = "b25b959554ed76058ac220b7b2e0a026",
  //url = "http://ws.audioscrobbler.com/2.0/?method=tag.getsimilar&tag=" + tag + "&api_key=" + key + "&format=json";
  //url = "http://ws.audioscrobbler.com/2.0/?method=user.gettopartists&user=" + user + "&api_key=" + key + "&format=json&limit=200";
  //tags = json.topartists.artist;
  //var url = "http://api.twitter.com/1/statuses/user_timeline.json?screen_name=" + encodeURIComponent(user) + "&count=200&trim_user=1";
  //var url = "http://search.twitter.com/search.json?rpp=100&q={word}" + encodeURIComponent(word);
  url = url.replace("{word}", encodeURIComponent(word))
  if (arguments.length < 3) attempts = 3;
  var params = {};
  if (!matchTwitter.test(url)) {
    params.url = url;
    url = "http://www.jasondavies.com/jsonp/";
  }
  statusText
      .style("display", null)
      .text("Fetching… " + (4 - attempts));
  getJSON(url, params, function(json) {
    if (typeof json === "string") parseHTML(json || "");
    else if (--attempts && json.location) getURL(json.location, word, attempts);
    else if ((json = json.results || json).length) parseText(json.map(function(d) { return d.text; }).join(" "));
    else statusText.text("Error fetching data!");
  }, function() {
    statusText
        .text("Error fetching data!");
    if (--attempts) getURL(url, word, attempts);
  });
}
